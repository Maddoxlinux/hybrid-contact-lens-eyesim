...
from ._opt import bisect, golden_section
...
def fit_base_power(eye_builder, pupil, lam_ref=LAMBDA_GREEN,
                   ...
                   diff_power=0.0):
    """Return the refractive base power (D) that sharpens the retinal image.

    `eye_builder(front_optics=...)` must return an EyeModel whose retina_z is
    fixed by the eye's anatomy.  `diff_power` is the diffractive add power that
    the final lens will carry: it is included here so the base is fitted for
    the *complete* lens (otherwise adding it later would re-defocus the eye).
    """
    from .metrics import spot_diagram      # local import avoids a cycle
    ...
    def defocus(P):
        lens = build_hybrid_lens(P, diff_power, design_lam, material)
        eye = eye_builder(front_optics=lens.surfaces)
        _, bfd = paraxial_power(
            System(eye.system.surfaces[:-1], eye.system.material_before), lam_ref)
        return z_post_lens + bfd - retina_z

    # Paraxial estimate (the lens front surface replaces the air-cornea
    # interface, so it carries ~40 D plus the correction).
    P_paraxial = bisect(defocus, -20.0, 120.0, xtol=1e-3)

    # The strong meniscus has spherical aberration, so the sharpest *real* focus
    # sits a few dioptres below the paraxial one. Refine on the real ray spot.
    def rms(P):
        lens = build_hybrid_lens(P, diff_power, design_lam, material)
        eye = eye_builder(front_optics=lens.surfaces)
        r = spot_diagram(eye.system, pupil, lam_ref, rings=8)["rms_um"]
        return r if np.isfinite(r) else 1e6

    return golden_section(rms, P_paraxial - 20.0, P_paraxial + 10.0, tol=0.05)


def auto_fit_lens(eye_builder, pupil, material="silicone-hydrogel",
                  design_lam=LAMBDA_GREEN, use_diffractive=True,
                  lam_ref=LAMBDA_GREEN):
    """Fit the hybrid lens in the correct order and return (base, diff) in D.

    1. rough base (no diffractive) to anchor the chromatic fit,
    2. diffractive add power that nulls the ocular chromatic aberration,
    3. final base power fitted *with* that diffractive present.
    """
    base_est = fit_base_power(eye_builder, pupil, lam_ref, material, design_lam)
    diff = (fit_diffractive_power(eye_builder, base_est, pupil, material, design_lam)
            if use_diffractive else 0.0)
    base = fit_base_power(eye_builder, pupil, lam_ref, material, design_lam,
                          diff_power=diff)
    return base, diff
